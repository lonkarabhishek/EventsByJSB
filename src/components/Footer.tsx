    ...
    <footer className="bg-foreground py-16 px-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                ...
                <Link key={link.to} to={link.to} className="block text-sm text-primary-foreground/50 hover:text-primary-foreground transition-colors font-body">
                  {link.label}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {["Wedding Planning", "Destination Weddings", "Engagement Ceremonies", "Corporate Events", "Private Celebrations"].map((s) => (
                <p key={s} className="text-sm text-primary-foreground/50 font-body">{s}</p>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-4">
              {[Instagram, Facebook, Twitter].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full border border-primary-foreground/15 flex items-center justify-center hover:border-primary-foreground/40 transition-colors">
        ...
        <div className="border-t border-primary-foreground/10 pt-8 text-center">
          <p className="text-sm text-primary-foreground/30 font-body">© {new Date().getFullYear()} JSB Events. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
